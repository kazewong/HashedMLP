...
writer.add_gradph(model,coord[1:2])
best_loss = 1e10
...
for epoch in range(n_epoch):
    ...
    for i in range(0,data.shape[0],batch_size):
        ...
        if running_loss < best_loss:
            ...
            writer.add_scalar('training_loss',running_loss*batch_size/data.shape[0],epoch*data.shape[0]+i)
            #torch.save(model.module.state_dict(), './checkpoint/Athena_turbulence_512_best.pth')
            torch.save(model.module.state_dict(), './checkpoint/Athena_mixing_layer_256_best.pth')
