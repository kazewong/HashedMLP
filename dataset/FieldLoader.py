...
class FieldIndexDataset(torch.utils.data.Dataset[Tuple[torch.Tensor, torch.Tensor]]):
    ...
    """
    def __init__(self, shape: Collection[int], normalize: bool=True):
        ...
        normalize : bool
            If `True`, the multi-index will be normalized to the range ``[0, 1]``
            in each dimension. Otherwise, the multi-index will be in the range
            of the grid size in each dimension.
        """
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        if self.normalize:
            multi_idx = multi_idx / self._shape

        return torch.from_numpy(multi_idx), torch.as_tensor(idx)


class FieldDataset(FieldIndexDataset):
    def __init__(self, field: torch.Tensor, normalize: bool=True):
        super().__init__(field.shape[:-1], normalize=normalize)
