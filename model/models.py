...
class Interpolator(torch.nn.Module):
    ...
    def __init__(self, grid_values: torch.nn.Module, grid_size: Sequence[int]):
        ...
        self.register_buffer('grids', torch.as_tensor(grid_size))
        self.register_buffer('grid_steps', 1 / self.grids)
    ...
    def linear_interpolation(self, position: torch.Tensor, lower_corner: torch.Tensor, corner_values: torch.Tensor) -> torch.Tensor:
        lower_limit = lower_corner.div(self.grids)
        upper_limit = lower_corner.add(1).div(self.grids)
    ...
    def forward(self, position: torch.Tensor) -> torch.Tensor:
        ...
        corner_index = torch.repeat_interleave(lower_corner.unsqueeze(-2), 2**len(self.grids), dim=1).add_(self.bit_table)
...
class MultigridInterpolator(torch.nn.Module):
    ...
    """
    def __init__(self, levels: int, entries_per_level: int, base_grid: Collection[int], final_grid: Collection[int], features: int=2, max_norm: float=None, norm_type: float=2, sparse: bool=False):
        ...
        base_grid = torch.as_tensor(base_grid)
        final_grid = torch.as_tensor(final_grid)
        factor = torch.exp((final_grid.log() - base_grid.log()) / (levels - 1))

        def _make_interpolator(level):
            grid_size = base_grid.mul(factor**level).round_().int()
            grid = make_grid_with_size(entries_per_level, features, grid_size, max_norm=max_norm, norm_type=norm_type, sparse=sparse)
            return Interpolator(grid, grid_size)
